...
async function loadPredAction(match, colors, isUpcoming = true) {
  ...
  // Formulaire de prédiction avec boutons de score valides
...
function selectDetailWinner(btn, winner, matchId, format, team1, team2) {
  ...
  const validScores = window.getValidScores ? getValidScores(format) : (format === 'Bo5' ? [[3,0],[3,1],[3,2]] : format === 'Bo1' ? [[1,0]] : [[2,0],[2,1]]);
