    ...
    <div class="sidebar-group">
      ...
      ${games.map(({ key, cfg }) => `
        <div class="game-toggle-row">
          ...
          ${window.ESPORT_INFO?.[key] ? `<button class="game-info-btn" onclick="event.stopPropagation();showEsportInfoPage('${key}')" title="Informations">📚</button>` : ''}
        </div>
      `).join('')}
    </div>
  `).join('');
    ...
    + Object.entries(byGenre).map(([genre, games]) => 
        ...
        + games.map(({ key, cfg }) => {
          ...
          return '<div class="drawer-game-item ' + (isActive ? 'active' : '') + '" onclick="selectGame(&quot;' + key + '&quot;);closeGameDrawer()">'
            ...
            + (window.ESPORT_INFO?.[key] ? '<button class="drawer-info-btn" onclick="event.stopPropagation();closeGameDrawer();showEsportInfoPage(&quot;' + key + '&quot;)" title="Informations">📚</button>' : '')
            + '</div>';
        }).join('')
      ).join('')
